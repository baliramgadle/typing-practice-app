...
const get_font_range = document.getElementById("font-range");
const display_font = document.getElementById("font-size");
...
get_font_range.addEventListener("change", (e) => {
  second.style.fontSize = get_font_range.value + "px";
  display_font.innerText = get_font_range.value + "px";
});
...
text2.addEventListener("input", () => {
  ...
  if (startTime === null) {
    startTime = Date.now();
    timer = setInterval(
      () => updateResult(correctChars, userInput, sampleText),
      100
    );
  }
...
text1.addEventListener("keydown", (e) => {
  if (text1.value.trim() == "!5" && e.key == "Enter") {
    ...
    text1.value =
      "Mental health is a fundamental aspect of overall well-being. Conditions like stress, anxiety, and depression affect many, yet often remain unspoken. Destigmatizing mental health and providing access to support and care is vital. Simple activities like exercising, talking to a friend, or practicing mindfulness can significantly improve mental well-being. Just as we care for our bodies, we must prioritize mental health to lead balanced and fulfilling lives.";
  } else if (text1.value.trim() == "!4" && e.key == "Enter") {
    text1.value =
      "Lifelong learning is the ongoing pursuit of knowledge throughout life. It helps individuals adapt to changing circumstances, whether in their careers or personal lives. Learning new skills, exploring different fields, or simply staying curious fosters growth and resilience. With countless resources available online and offline, lifelong learning is accessible to anyone. It not only enhances professional skills but also enriches personal experiences and promotes continuous self-improvement.";
  }
});
